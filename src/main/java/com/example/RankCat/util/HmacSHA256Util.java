...
public final class HmacSHA256Util {
    ...
    public static String sign(
            String secretKey, String timestamp, String method, String path) { // 네이버에서 요구하는 서명 문자열
        try {
            String msg = String.join(".", timestamp, method, path);
            Mac mac = Mac.getInstance(ALG);
            mac.init(
                    new SecretKeySpec(
                            secretKey.getBytes(StandardCharsets.UTF_8), ALG)); // 비밀키로 HMAC초기화
            return Base64.getEncoder()
                    .encodeToString(
                            mac.doFinal(
                                    msg.getBytes(
                                            StandardCharsets
                                                    .UTF_8))); // 서명을 Base64 인코딩해 헤더에 넣을 문자열로 반환
        } catch (Exception e) {
