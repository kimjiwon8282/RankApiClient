...
public class RestTemplateConfig {
    ...
    public RestTemplate naverRestTemplate() { // 커스텀 RestTemplate 빈 등록
        ...
        RestTemplate rt =
                new RestTemplate( // RestTemplate에 커스텀 HTTP 엔진 연결
                        new HttpComponentsClientHttpRequestFactory(client));

        // 기본 Base URL 세팅-> 이후 서비스에서 경로만 넘기면 /keywordstool처럼 사용
        rt.setUriTemplateHandler(new DefaultUriBuilderFactory("https://api.naver.com"));
        // 공통 헤더 인터셉터 + 호출마다 공통 헤더를 삽입함
        rt.getInterceptors()
                .add(
                        (req, body, ex) -> { // 매 요청을 보내기 직전 공통 로직 수행
                            String timestamp =
                                    String.valueOf(
                                            System.currentTimeMillis()); // 네이버 api요구 헤더용 타임스탬프
                            String signature =
                                    com.example.RankCat.util.HmacSHA256Util.sign(
                                            adprops.getSecretKey(),
                                            timestamp,
                                            req.getMethod().name(),
                                            req.getURI().getPath());
    ...
    public RestTemplate shoppingInsightRestTemplate() {
        ...
        RestTemplate rt =
                new RestTemplate( // 커스텀 HTTP엔진 연결
                        new HttpComponentsClientHttpRequestFactory(client));
        rt.setUriTemplateHandler(new DefaultUriBuilderFactory("https://openapi.naver.com"));
        rt.getInterceptors()
                .add(
                        (req, body, ex) -> {
