...
public class KeywordToolController {
    ...
    public ResponseEntity<?> keywordSearch(@RequestParam String keyword) {
        if (keyword == null || keyword.isBlank()) {
            return ResponseEntity.badRequest().body("키워드를 입력하세요.");
        }
        try {
            return ResponseEntity.ok(keywordToolService.getRelatedKeywords(keyword));
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body("API 호출 실패: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<SearchAdKeywordResult> getKeywordAnalysis(@RequestParam String query) {
        ...
        return resultOpt.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
}
